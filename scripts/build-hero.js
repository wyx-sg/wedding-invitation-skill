// Compose the 15 example thumbnails into a single hero image for the README.
...
const ROWS = 3;
const CELL_W = 280;
const CELL_H = 373; // 280 × (1120/840) = 373.3
const GAP = 18;
const PAD = 40;
...
function main() {
  ...
  if (thumbs.length !== COLS * ROWS) {
    console.warn(`[hero] Expected ${COLS * ROWS} thumbnails, found ${thumbs.length}. Layout may have empty cells.`);
  }

  const cards = thumbs.map(f => {
